...
class thread1 extends Thread {
	...
	String tname;
	check c;
	thread1(String name,check c1) {
		tname=name;
		c=c1;
	}
	public void run() {
		synchronized(c) {                              //synchronized threads
			c.countdown();
		}
		System.out.println(tname+" Countdown over");
	}
	public void start() {
		System.out.println(tname+" Countdown begins");
		if(t==null) {
...
public class GUI {
	...
	public static void main(String[] args) {
		check c=new check();
		thread1 t1=new thread1("Thread1",c);
		thread1 t2=new thread1("Thread2",c);
		t1.start();
		t2.start();
		try {
			t1.join();
